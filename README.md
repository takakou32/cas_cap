...
| 実装 | 場所 | 必要なもの | 特徴 |
|------|------|-----------|------|
| **PowerShell** | `powershell/cdp_capture.ps1` | Windows標準のPowerShellのみ | **外部依存ゼロ**。.NETの`ClientWebSocket`でCDPを直接操作 |
| **JavaScript** | `js/cdp_capture.js` | Node.js + Playwright | Playwrightベースで安定。`networkidle`待機などをフルサポート |
| Python（オリジナル） | `src/cdp_capture.py` | Python + Playwright | 元実装 |
