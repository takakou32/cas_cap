...
| `near_text` | ボタン名が無いクリック（アイコン等）の、近くの見出し（記録時に自動で付く）。位置がずれても見出しが同じ要素を押す |
...
| 宛名番号を含まない | 記録位置＋ボタン名 → ボタン名で探す。見つからなければ失敗 |
| ボタン名が無い（入力欄の横の虫眼鏡アイコン等）で、近くの見出しを記録してある | 記録位置・位置の番号を外したセレクタ・要素の種類だけ（例 `i.pi.pi-search-plus.icons`）で広く探し、**近くの見出しが記録と同じもの**を押す。人によって欄の数が違い位置がずれても、同じ欄のアイコンを押せる。見出しが同じ候補が2つ以上なら件数を出して失敗 |
| ボタン名が無く、見出しも無い（以前の版で録った記録・近くに見出しが無い） | 記録位置の要素を押す。無ければ、同じ種類の要素が画面に**ちょうど1つ**の時だけ押す。2つ以上なら件数を出して失敗 |

「近くの見出し」は、アイコンから数階層さかのぼって最初に見つかる `label` 要素の文字（記録ファイルでは `near_text`）。入力値は人ごとに変わるので使わない。以前の版で録った記録には無いので、欄の数が人によって違う画面のアイコンは録り直すこと（録り直さないと、位置が合う別の欄のアイコンを押すことがある）。
...
`test/mock/search.html` は、宛名検索でよくある作り（宛名番号を打つとサジェストが出る／検索ボタンで対象者一覧（前方一致）を出す／行を選ぶと詳細へ進み、URL には世帯番号が入る／詳細の欄の横に文字の無い虫眼鏡アイコンがあり、欄の数が人によって違う／「帳票」タブで約3秒くるくるが出て、その間はメニューが効かない）を再現した練習ページ。記録や再生の動きを本物の前に確かめる時に使う。
