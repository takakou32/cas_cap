...
2. 確認画面を回る操作を、バッチ用として記録する
   ...
   2. **対象者ごとに表示が変わる操作はどれですか？**（`-ClickNav` の時だけ） — 候補や一覧の表示名が**氏名だけ**（宛名番号を含まない）で、それを押す操作を選ぶ。
      選んだ操作には `by_index` が付き、ボタン名（記録時の人の氏名）は保存しない。再生時は候補が**ちょうど1件**の時だけ押す
      - 表示名に宛名番号が含まれる候補（例「11111 山田 太郎」）は選ばなくてよい（新しい宛名番号で探すため）
      - クリック以外の番号は無視される
...
| 宛名番号を含まない | 記録位置＋ボタン名 → ボタン名で探す。見つからなければ失敗（別の要素で代わりに押すことはしない） |
| `by_index` を付けた操作（氏名だけの候補など） | ボタン名は使わない。記録した要素と同じ種類（位置の番号は無視）が画面に**ちょうど1件**見えている時だけ押す。2件以上なら件数を出して失敗、0件でも失敗 |
...
| 入力欄やクリック対象が見つからない・1つに決められない（`by_index` で2件以上を含む）・待ちが時間切れ・例外 | **その件をその場で打ち切って**次の件へ（ずれた画面のまま撮らない）。それまでに撮れた画像は残る |
...
`test/mock/suggest.html` は、宛名番号サジェストでよくある作り（キーを離した時だけ検索する／マウスを押した瞬間に候補を選ぶ／候補の位置が毎回変わる／選択後に表示名を書き戻す／URL に世帯番号が入る／「帳票」タブで約3秒くるくるが出て、その間はメニューが効かない）を再現した練習ページ。記録や再生の動きを本物の前に確かめる時に使う。
