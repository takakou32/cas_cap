...
ページごとに `actions` を実行した後で1枚撮る。`"capture": false` を付けたページは操作だけ行い、撮らない。
...
2. 確認画面を回る操作を、バッチ用として記録する
   ...
   記録を終えると、次の順に質問される（手順の一覧が番号つきで出るので、番号をカンマ区切りで答える。無ければ Enter）:

   1. **画面コピーを取るのはどの操作の後ですか？** — 撮りたい画面が表示される操作を選ぶ（例: 検索ボタン、帳票タブ）。
      選んだ操作の後で1枚ずつ撮る。**Enter だけなら、クリック・画面遷移ごとにすべて撮る**（従来どおり）
      - 選んだ操作に次の「待たせる操作」も付けた場合は、待ってから撮る
      - 最後に選んだ操作より後の操作（プレビューを閉じる等）は、撮らずに操作だけ行う（記録ファイルでは `"capture": false` のページ）
      - 画像のページ名は撮る順の連番になる（`recorded_001`, `recorded_002`, …）
   2. **実行後に待たせる操作はどれですか？** — 帳票プレビューの作成など、押した後しばらく「くるくる」が出る操作を選ぶ。
   ...
   3. **対象者ごとに表示が変わる操作はどれですか？**（`-ClickNav` の時だけ） — 候補や一覧の表示名が**氏名だけ**（宛名番号を含まない）で、それを押す操作を選ぶ（対象者一覧の行など）。
...
```
output\batch_{実行日時}\{チェック項目}\{宛名番号}_{大分類}_{小分類}_{ページ名}.png
```

例: `output\batch_20260916_153000\04_7_印鑑_最大文字調査_外国人住所最大\222222_04_7_recorded_002.png`

- フォルダはチェック項目（CSV の1列目）ごと。ファイル名に使えない文字は `_` に置き換える
